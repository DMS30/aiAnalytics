...
@interface aiContext () <PHGMutableContext>
...
{
    // We're also being a bit clever here by implementing aiContext actually as a mutable
    ...
    aiContext *context = self.debug ? [self copy] : self;
    modify(context);
...
{
    aiContext *ctx = [[aiContext allocWithZone:zone] initWithPostHog:self._posthog];
