...
@interface aiPostHogConfiguration ()
...
@implementation aiPostHogConfiguration

+ (instancetype)configurationWithApiKey:(NSString *)apiKey
{
    return [[aiPostHogConfiguration alloc] initWithApiKey:apiKey host:@"https://app.posthog.com"];
}

+ (instancetype)configurationWithApiKey:(NSString *)apiKey host:(NSString *)host
{
    return [[aiPostHogConfiguration alloc] initWithApiKey:apiKey host:host];
}
...
- (instancetype)init
{
    if (self = [super init]) {
        ...
        self.libraryVersion = [aiHog version];
