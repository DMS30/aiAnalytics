...
#import "aiHogUtils.h"
#import "aiHog.h"
#import "UIViewController+aiScreen.h"
...
static aiHog *__sharedInstance = nil;
...
@property (nonatomic, strong) aiStoreKitCapturer *storeKitCapturer;
@property (nonatomic, strong) aiPayloadManager *payloadManager;
@property (nonatomic, strong) PHGMiddlewareRunner *runner;

@end


@implementation aiHog

+ (void)setupWithConfiguration:(aiPostHogConfiguration *)configuration
{
...
- (instancetype)initWithConfiguration:(aiPostHogConfiguration *)configuration
{
    ...
    if (self = [self init]) {
        ...
        self.payloadManager = [[aiPayloadManager alloc] initWithPostHog:self];
        ...
        if (configuration.captureInAppPurchases) {
            _storeKitCapturer = [aiStoreKitCapturer captureTransactionsForPostHog:self];
        }
...
- (void)identify:(NSString *)distinctId properties:(NSDictionary *)properties options:(NSDictionary *)options
{
    ...
    [self run:PHGEventTypeIdentify payload:
                                       [[aiIdentifyPayload alloc] initWithDistinctId:distinctId
                                                                          anonymousId:anonId
...
- (void)capture:(NSString *)event properties:(NSDictionary *)properties
{
    ...
    [self run:PHGEventTypeCapture payload:
                                    [[aiCapturePayload alloc] initWithEvent:event
...
- (void)screen:(NSString *)screenTitle properties:(NSDictionary *)properties
{
    ...
    [self run:PHGEventTypeScreen payload:
                                     [[aiScreenPayload alloc] initWithName:screenTitle
                                                                 properties:PHGCoerceDictionary(properties)]];
}
...
- (void)alias:(NSString *)alias
{
    [self run:PHGEventTypeAlias payload:
                                    [[aiAliasPayload alloc] initWithAlias:alias]];
}
...
//    TODO: set groups as super property
    ...
    [self run:PHGEventTypeGroup payload: [[aiGroupPayload alloc] initWithType:groupType groupKey:groupKey properties:PHGCoerceDictionary(properties)]];
...
- (id)getFeatureFlag:(NSString *)flagKey options:(NSDictionary *)options
{
    ...
    if (send_event == nil || [send_event boolValue] != false) {
        ...
        [self run:PHGEventTypeCapture payload:
                                        [[aiCapturePayload alloc] initWithEvent:@"$feature_flag_called"
                                                                    properties:PHGCoerceDictionary(properties)]];
    }
...
- (void)continueUserActivity:(NSUserActivity *)activity
{
    ...
    if ([activity.activityType isEqualToString:NSUserActivityTypeBrowsingWeb]) {
        ...
        properties = [aiUtils traverseJSON:properties
...
- (void)openURL:(NSURL *)url options:(NSDictionary *)options
{
    ...
    payload.url = [NSURL URLWithString:[aiUtils traverseJSON:url.absoluteString
                                        andReplaceWithFilters:self.configuration.payloadFilters]];
...
- (void)run:(PHGEventType)eventType payload:(aiPayload *)payload
{
    if (!self.enabled) {
        return;
    }
    aiContext *context = [[[aiContext alloc] initWithPostHog:self] modify:^(id<PHGMutableContext> _Nonnull ctx) {
