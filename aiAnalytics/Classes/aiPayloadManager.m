...
#import "aiStorage.h"
#import "aiFileStorage.h"
#import "aiUserDefaultsStorage.h"
#import "aiPayloadManager.h"
#import "aiHogIntegration.h"

NSString *aiHogIntegrationDidStart = @"com.posthog.integration.did.start";
static NSString *const PHGAnonymousIdKey = @"PHGAnonymousId";
static NSString *const kPHGAnonymousIdFilename = @"posthog.anonymousId";


@interface aiPayloadManager ()

@property (nonatomic, strong) aiHog *posthog;
@property (nonatomic, strong) aiPostHogConfiguration *configuration;
@property (nonatomic, strong) dispatch_queue_t serialQueue;
@property (nonatomic, strong) NSMutableArray *messageQueue;
@property (nonatomic, strong) aiHogIntegration *integration;
...
@property (nonatomic, strong) id<aiStorage> userDefaultsStorage;
@property (nonatomic, strong) id<aiStorage> fileStorage;

@end


@implementation aiPayloadManager

- (instancetype _Nonnull)initWithPostHog:(aiHog *_Nonnull)posthog
{
    aiPostHogConfiguration *configuration = posthog.configuration;
    ...
    if (self = [super init]) {
        ...
        self.httpClient = [[aiHTTPClient alloc] initWithRequestFactory:configuration.requestFactory];
        
        self.userDefaultsStorage = [[aiUserDefaultsStorage alloc] initWithDefaults:[NSUserDefaults standardUserDefaults] namespacePrefix:nil crypto:configuration.crypto];
        #if TARGET_OS_TV
            self.fileStorage = [[aiFileStorage alloc] initWithFolder:[aiFileStorage cachesDirectoryURL] crypto:configuration.crypto];
        #else
            self.fileStorage = [[aiFileStorage alloc] initWithFolder:[aiFileStorage applicationSupportDirectoryURL] crypto:configuration.crypto];
        #endif

        self.cachedAnonymousId = [self loadOrGenerateAnonymousID:NO];
    
        self.integration = [[aiHogIntegration alloc] initWithPostHog:self.posthog httpClient:self.self.httpClient fileStorage:self.fileStorage userDefaultsStorage:self.userDefaultsStorage];
        
        if (self.integration.distinctId.length == 0) {
            [self.integration saveDistinctId:self.cachedAnonymousId];
        }

        [[NSNotificationCenter defaultCenter] postNotificationName:aiHogIntegrationDidStart object:@"PostHog" userInfo:nil];
...
- (void)identify:(NSString *)distinctId properties:(NSDictionary *)properties options:(NSDictionary *)options
{
    ...
    aiIdentifyPayload *payload = [[aiIdentifyPayload alloc] initWithDistinctId:distinctId
...
- (void)capture:(NSString *)event properties:(NSDictionary *)properties options:(NSDictionary *)options
{
    ...
    aiCapturePayload *payload = [[aiCapturePayload alloc] initWithEvent:event
                                                               properties:PHGCoerceDictionary(properties)];
...
- (void)screen:(NSString *)screenTitle properties:(NSDictionary *)properties options:(NSDictionary *)options
{
    ...
    aiScreenPayload *payload = [[aiScreenPayload alloc] initWithName:screenTitle
                                                            properties:PHGCoerceDictionary(properties)];
...
{
    aiAliasPayload *payload = [[aiAliasPayload alloc] initWithAlias:alias];
...
{
    aiGroupPayload *payload = [[aiGroupPayload alloc] initWithType:groupType groupKey:groupKey
                                                               properties:PHGCoerceDictionary(properties)];
...
- (void)invokeIntegration:(aiHogIntegration *)integration key:(NSString *)key selector:(SEL)selector arguments:(NSArray *)arguments options:(NSDictionary *)options
{
...
{
    NSMethodSignature *signature = [aiHogIntegration instanceMethodSignatureForSelector:selector];
...
@interface aiPayload (Options)
@property (readonly) NSDictionary *options;
@end


@implementation aiPayload (Options)
...
@implementation aiPayloadManager (aiMiddleware)

- (void)context:(aiContext *)context next:(void (^_Nonnull)(aiContext *_Nullable))next
{
    switch (context.eventType) {
        case PHGEventTypeIdentify: {
            aiIdentifyPayload *p = (aiIdentifyPayload *)context.payload;
            NSDictionary *options;
            if (p.anonymousId) {
