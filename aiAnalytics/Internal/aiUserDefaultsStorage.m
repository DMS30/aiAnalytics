...
@implementation aiUserDefaultsStorage

- (instancetype)initWithDefaults:(NSUserDefaults *)defaults namespacePrefix:(NSString *)namespacePrefix crypto:(id<aiCrypto>)crypto
{
    if (self = [super init]) {
        _defaults = defaults;
...
- (id _Nullable)plistForKey:(NSString *)key
{
    ...
    return data ? [aiUtils plistFromData:data] : nil;
}

- (void)setPlist:(id _Nonnull)plist forKey:(NSString *)key
{
    NSData *data = [aiUtils dataFromPlist:plist];
    if (data) {
