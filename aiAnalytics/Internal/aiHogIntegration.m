...
NSString *const aiHogDidSendRequestNotification = @"PostHogDidSendRequest";
NSString *const aiHogRequestDidSucceedNotification = @"PostHogRequestDidSucceed";
NSString *const aiHogRequestDidFailNotification = @"PostHogRequestDidFail";
...
@property (nonatomic, strong) aiReachability *reachability;
@property (nonatomic, strong) NSTimer *flushTimer;
@property (nonatomic, strong) dispatch_queue_t serialQueue;
@property (nonatomic, strong) dispatch_queue_t backgroundTaskQueue;
@property (nonatomic, assign) aiHog *posthog;
@property (nonatomic, assign) aiPostHogConfiguration *configuration;
...
@implementation aiHogIntegration
...
- (id)initWithPostHog:(aiHog *)posthog httpClient:(aiHTTPClient *)httpClient fileStorage:(id<aiStorage>)fileStorage userDefaultsStorage:(id<aiStorage>)userDefaultsStorage;
{
    if (self = [super init]) {
        ...
        self.reachability = [aiReachability reachabilityWithHostname:@"google.com"];
...
- (NSDictionary *)staticContext
{
    ...
    NSArray<UIWindow *> *appWindows = [aiApplicationUtils.sharedInstance windows];
...
- (void)sendData:(NSArray *)batch
{
    ...
    self.batchRequest = [self.httpClient upload:payload host:self.configuration.host completionHandler:^(BOOL retry) {
        [self dispatchBackground:^{
            if (retry) {
                [self notifyForName:aiHogRequestDidFailNotification userInfo:batch];
            ...
            [self persistQueue];
            [self notifyForName:aiHogRequestDidSucceedNotification userInfo:batch];
            self.batchRequest = nil;
            [self endBackgroundTask];
        }];
    }];

    [self notifyForName:aiHogDidSendRequestNotification userInfo:batch];
}
